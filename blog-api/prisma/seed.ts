...
async function main() {
  ...
  // ── Authors ───────────────────────────────────────────────────────────────
  const maraVoss = await prisma.author.upsert({
    where: { id: "author-mara" },
    update: {},
    create: {
      id: "author-mara",
      name: "Mara Voss",
      initials: "MV",
      bio: "Contributing editor at The Margin. Writes about culture, technology, and the spaces in between.",
    },
  });

  const samOkafor = await prisma.author.upsert({
    where: { id: "author-sam" },
    update: {},
    create: {
      id: "author-sam",
      name: "Sam Okafor",
      initials: "SO",
      bio: "Engineer and writer. Interested in learning, craft, and how knowledge compounds over time.",
    },
  });

  const lenaPark = await prisma.author.upsert({
    where: { id: "author-lena" },
    update: {},
    create: {
      id: "author-lena",
      name: "Lena Park",
      initials: "LP",
      bio: "Product manager and occasional essayist. Focused on the human side of work.",
    },
  });
  ...
  const tagSlugs = [
    ...
    { name: "self-knowledge", slug: "self-knowledge" },
  ];
